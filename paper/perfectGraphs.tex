...
\begin{algorithmic}[1]
	...
	\mForEach{triangle $b_1, b_2, b_3$}
		\mForEach{$s_1, s_2, s_3$, such that for $1 \leq i < j \leq 3$, $\{b_i, s_i\}$ is disjoint
			\lsx  from $\{b_j, s_j\}$ and $b_ib_j$ is the only edge between them}
			\mIf{there is a vertex $a$, adjacent to all of $s_1, s_2, s_3$, and to at most
				\lsx one of $b_1, b_2, b_3$, such that if $a$ is adjacent to $b_i$, then $b_i = s_i$ \lsx}
				...
				\mForEach{$m_1 \in M \cup \{b_1\}$} \Comment{calculate good $(1, 2)$-pairs}
					\mIf{$P_1(m_1) \neq \UND$}
						...
						\mForEach{$m_2 \in M \cup \{b_2\}$}
							\mIf{$P_2(m_2)$ exists and contains no black vertices}
				...
				\mForEach{triple $m_1, m_2, m_3$ such that $m_i \in M \cup \{b_i\}$}
