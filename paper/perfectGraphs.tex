...
\begin{theorem}[Perfect graph theorem from \cite{LL72}]
	\label{thm:pgt}
...
The following is based on the paper by \citeauthor{MC05} \citetitle*[]{MC05} \cite{MC05}. We will not provide full proof of its correctness, but we will aim to show the intuition behind the algorithm.
...
Some pictures might be helpful. \Cref{fig:amenableOddHole} shows an example of an amenable odd hole $C = v_1, \ldots, v_7$. It is not a clean hole -- vertices $v_8$ and $v_9$ are $C$-major, and $v_8, v_9, v_1, v_2, v_3$ is an axample of a near-cleaner for $C$.
\Cref{fig:notAmenableHole} shows an odd hole $C = v_1, \ldots, v_7$ that is not amenable, because there is no $\{v_8, v_9\}$-complete edge in $C$.
...
\begin{theorem}[Theorem 9.2 of \cite{MC05}]
	\label{thm:92}
	Let $G$ be a graph, such that $G$ and $\overline{G}$ contain no Pyramid, no Jewel and no configuration of types $\T_1, \T_2$ or $\T_3$. Then the result of the \Cref{alg:listNearCleaners} on the graph $G$ is a set $\mathcal{R}$, such that for every shortest amenable hole $C$, there is a near-cleaner for $C$ in $\mathcal{R}$.
\end{theorem}
\begin{proof}
			...
			\mForEach{$y_1 \in V(G) \setminus X$} \label{line:cudaStart}
				\mForEach{3-vertex path $x_1-x_2-x_3$ of $G\setminus y_1$} \label{line:3vertex}
					\mIf{$R(x_1, y_1), R(x_2, y_2)$ both exist} \label{line:cudaWorkStard}
						...
						\mIf{$r(x_2, y_1) = r(x_1, y_1) + 1 = r(x_1, y_2) =$\lsx$min(r(x_3, y_1), r(x_3, y_2))$}
...
\begin{theorem}[Theorem 5.1 of \cite{MC05}]
	...
	Let $G$ be a graph, such that $G$ and $\overline{G}$ contain no Pyramid, no Jewel and no configuration of types $\T_1, \T_2$ or $\T_3$. Let $X$ be a subset of $V(G)$. If \Cref{alg:testNearCleaner} reports an odd hole, then $G$ contains an odd hole, else there is no shortest odd hole $C$ in $G$, such that $X$ is a near-cleaner for $C$.
\end{theorem}
\begin{proof}
...
\subsection{Algorithm for Berge graphs recognition}

To summarize, we present a complete algorithm for recognizing Berge graphs.
\begin{alg}[Determine if a graph is Berge]
	\label{alg:testNearCleaner}
	Input: A graph $G$.

	\noindent Output: Determines if $G$ is Berge.
	\label{alg:bergeness}
\end{alg}

\begin{algorithmic}[1]
	\mProcedure{Check-Berge}{$G$}
		...
		\mForEach{$X \in \mathcal{R'}$}
			...
			\mEndIf
		\mEndFor
		\ls \RETURN $G$ is Berge
	\mEndProcedure
\end{algorithmic}

\begin{theorem}[Theorem 10.1 of \cite{MC05}]
	Given a graph $G = (V, E)$, \Cref{alg:bergeness} works in $O(|V|^9)$ time and determines is $G$ is Berge.
\end{theorem}
\begin{proof}
