...
The following is based on the paper by \citeauthor{MC05} \citetitle*[]{MC05} \cite{MC05}. We will not provide full proof of its correctness, but will aim to show the intuition behind the algorithm.
	...
	\mForEach{$v_1, v_2, v_5 \in V(G)$, so that $v_1v_2$ is an edge \AND
		...
		\mForEach{$X$ -- an anticomponent of $Y$}
			...
			\lsx and not to $v_2$ and $v_5$}
				\mIf{$v_4$ has a neighbor in $F$ and a nonneibhbor in $X$ \label{line:t3v4if}}
	...
	\mProcedure{Test-Near-Cleaner}{$G, X$}
		\mForEach{pair $x, y \in V(G)$, such that $x \neq y$}
			...
			\mElse
				...
				\ls $r(x, y) \gets \infty$
			\mEndIf
			\mForEach{$y_1 \in V(G) \setminus X$}
				\mForEach{3-vertex path $x_1-x_2-x_3$ of $G\setminus y_1$}
					\mIf{$R(x_1, y_1), R(x_2, y_2)$ both exist}
...
\begin{theorem}[Theorem 5.1 of \cite{MC05}]
	Let $G$ be a graph, such that $G$ and $\overline{G}$ contain no Pyramid, no Jewel and no configuration of types $\T_1, \T_2$ or $\T_3$. Let $X$ be a subset of $V(G)$. If \Cref{alg:testNearCleaner} reports an odd hole, then $G$ contains an odd hole.
\end{theorem}
\begin{proof}
