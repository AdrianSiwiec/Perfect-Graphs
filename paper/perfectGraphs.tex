...
\begin{theorem}
	...
	A graph $G$ is perfect iff for every induced subgraph $H$, the number of vertices of $H$ is at most $\alpha(H)\omega(H)$. \todo{proof?}
\end{theorem}
...
In 1961 Berge conjured that a graph is perfect iff it contains no odd hole and no odd antihole in what has become known as a strong perfect graph conjecture. In 2001 Chudnovsky et al. have proven it and published the proof in an over 150 pages long paper \citetitle{MC06} \cite{MC06}.
...
% 	\SetKwFunction{ContainsPyramid}{\textsc{Contains-Pyramid}}
% 	% \Indm\nonl\ContainsPyramid{$G$}\\
% 	\KwData{$G = (V, E)$ -- a graph}
% 	\KwResult{Whether $G$ contains a pytamid as an induced subgraph.}
% 	\Indp

% 	\ForEach{ triangle $b_1, b_2, b_3$ }{
% 		\ForEach{ $s_1, s_2, s_3$, such that for $1 \leq i < j \leq 3$, $\{b_i, s_i\}$ is disjoint from $\{b_j, s_j\}$ and $b_ib_j$ is the only edge between them }{
% 			\If{ there is a vertex $a$, adjacent to all of $s_1, s_2, s_3$, and to at most one of $b_1, b_2, b_3$, such that if $a$ is adjacent to $b_i$, then $b_i = s_i$ } {
% 				$M \gets V(G) \setminus \{b_1, b_2, b_3, s_1, s_2, s_3\}$\;
% 				\ForEach{$m \in M$} {
% 					$S_1(m) \gets$ the shortest path between $s_1$ and $m$ such that $s_2, s_3, b_2, b_3$ have no neighbors in its interior, if such a path exists. \;
% 					Calculate $S_2(m), S_3(m)$ similarly\;
% 					$T_1(m) \gets$ the shortest path between $m$ and $b_1$, such that $s_2, s_3, b_2, b_3$ have no neighbors in its interior, if such a path exists\;
% 					Calculate $T_2(m), T_3(m)$ similarly\;
% 				}
% 				\ForEach{$m \in M \cup \{b_1\}$}{
% 					\If{$s_1 = b_1$}{
% 						$P_1(b_1) \gets$ the one-vertex path with vertex $b_1$\;
% 						$\forall_{m \in M}: P_1(m) \gets$ \textsc{undefined}\;
% 					} \Else{
% 						$P_1(b_1) \gets $ \textsc{undefined}\;
% 						\ForEach{$m \in M$}{
