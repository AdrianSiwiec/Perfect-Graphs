...
There has been much progress on the quest of finding an algorithm coloring perfect graphs, without the use of ellipsoid method (see \Cref{sec:classicalColoring}), however there is still no known polynomial combinatorial algorithm to do this.
...
$$\sum\limits_{i\in S}(e_1^\intercal v_i)^2 \leq \sum\limits_{i \in S'}(e_1^\intercal v_i)^2 \leq ? = 1$$
\todo{todo}
Each term on the left hand side is $5^{-k/4}$, so the left hand side is equal to $|S|5^{-k/2}$, and so $|S| \leq 5^{k/2}$. Since $|S|$ was an arbitrary stable set, we get $\alpha(C_5^k) \leq 5 ^{k/2}$ and $\Theta(C_5) = \sqrt{5}$.
...
Note that coloring $C'$ uses $\chi(G \setminus S) = \omega(G \setminus S) = \omega(G) -1$. We will call recursion at most $O(|V|)$ times, each step of recursion requires polynomial time. Therefore the total running time of \Cref{alg:minColoring} is polynomial.
...
In 2005 Maffray and Trotignon showed a coloring algorithm that colors graphs containing no odd hole, no antihole and no prism (sometimes called Artemis graphs) in $O(|V|^4|E|)$ time \cite{Maffray2006}. They later improved the time complexity to $O(|V|^2|E|)$ \cite{coloringArtemis}.
